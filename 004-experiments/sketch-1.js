...
const sketch = () => {
    ...
    if (sketchValues.length == 0) {
      for(let i=0; i<num; i++) {
        sketchValues.push({
          ...
          scaleY: random.range(.2, .5),
          rectY: random.range(0, -h*.5),
          lineWidth: random.range(0.046*h, 0.185*h),
          radius: random.range(.5, 1.3),
    ...
    for(let i=0; i<num; i++) {
      ...
      let angle = slice * i * (playhead <= .125 ? playhead*8. : 1);
